-- ============================================================================
-- MIGRATION : CORRECTION ENREGISTREMENT LOCALISATION (RETRAIT DE CONTRIBUABLES)
-- ============================================================================

-- 1. Ajout de la colonne localisation à la table tps_liquidations
ALTER TABLE public.tps_liquidations ADD COLUMN IF NOT EXISTS localisation TEXT;

-- 2. Suppression des colonnes de localisation obsolètes de la table contribuables (TFU)
ALTER TABLE public.contribuables DROP COLUMN IF EXISTS commune;
ALTER TABLE public.contribuables DROP COLUMN IF EXISTS arrondissement;
ALTER TABLE public.contribuables DROP COLUMN IF EXISTS quartier;

-- 3. Suppression des colonnes de localisation obsolètes de la table tps_contribuables (TPS)
ALTER TABLE public.tps_contribuables DROP COLUMN IF EXISTS commune;
ALTER TABLE public.tps_contribuables DROP COLUMN IF EXISTS arrondissement;
ALTER TABLE public.tps_contribuables DROP COLUMN IF EXISTS quartier;
ALTER TABLE public.tps_contribuables DROP COLUMN IF EXISTS localisation;


-- 4. Redéfinition de creer_liquidation (TFU)
...
BEGIN
  ...
  -- Insérer ou récupérer le contribuable sans colonnes de localisation
  ...
  -- Enregistrer la localisation uniquement dans la table LIQUIDATIONS
...
-- 5. Redéfinition de creer_liquidation_tps (TPS)
...
DECLARE
  ...
  v_count INTEGER;
BEGIN
  -- Insérer ou mettre à jour le contribuable sans colonnes de localisation
  ...
  -- Enregistrer la localisation uniquement dans la table TPS_LIQUIDATIONS
...
-- 6. Redéfinition de valider_paiement_liquidation (TFU) sans dépendre de v_contrib.commune
...
-- 7. Redéfinition de valider_paiement_tps (TPS) sans dépendre de v_contrib.commune
